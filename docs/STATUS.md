...
impact band. Items marked *(audit 2026-07-18)* come from the current
audit; *(prior audit)* items are the still-true remainder of
`docs/retired/AUDIT.md`, re-verified against current code — the old
blanket "resolve remaining AUDIT.md findings" task is superseded by
these explicit entries. The `.context` orphan-reconciliation finding is
not listed: it is being fixed now by a dedicated agent.
...
| create prompting flow for initial request | turn status items into full prompts | high | medium |
...
| add visual feedback during context generation | medium | easy |
...
| integrate status more closely into the agent flow | surface `docs/STATUS.md` priorities and constraints in prompt injection; make agent aware of ability to add tasks to persistent doc | medium | medium |
