...
environment. Your working directory is the real project tree, mounted
read-write at its real path — use your native Read/Edit/Write/Grep
tools normally. There is no scratch copy: your edits land in the real
...
- Only the project workspace is writable (plus `/tmp`). The toolchain
  is read-only.
