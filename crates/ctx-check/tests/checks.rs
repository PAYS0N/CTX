...
use ctx_check::checks::run_selected;
use ctx_check::error::CheckError;
use ctx_check::model::Status;
use ctx_check::parse::{parse_compiler_json, parse_fail_lines, parse_fmt};
use ctx_check::runner::{CommandOutcome, Runner};
...
fn missing_tool_is_skipped_not_failed() {
    ...
    let report = run_selected(&runner, 20, Some(&["fmt".to_owned()]));
...
fn diagnostics_are_capped_and_counted() {
    ...
    let report = run_selected(&runner, 2, Some(&["clippy".to_owned()]));
...
#[test]
fn command_failure_without_diagnostics_still_fails() {
    // A compile abort can exit non-zero while emitting no parsable
    // compiler-message JSON; that must still report Fail, not Pass.
    let mut map = BTreeMap::new();
    map.insert(
        "clippy".to_owned(),
    ...
    let runner = FakeRunner { by_first_arg: map };
    let report = run_selected(&runner, 20, Some(&["clippy".to_owned()]));
    let clippy = report.checks.get("clippy").expect("clippy present");
    assert_eq!(clippy.status, Status::Fail);
    assert_eq!(clippy.count, 0);
    assert_eq!(report.status, Status::Fail);
}
